  ...
  #'  Parsons, Michael Havrda, & Shannon Kachel, UW SEFS.
  #'  =============================================================
  ...
  #'  Use Data_formatting_cattle_hunter_activity.R to generate DH for these
  ...
  #'  Lump all filtered data together
  CamTrap_Detections <- as.data.frame(rbind(images_graze2018, images_hunt2018,
    ...
    filter(Species == "Moose" | Species == "Mule Deer" | Species == "Elk" | 
  ...
  #'  -day1: sampling occasion begins on a specific date ("2018-07-01")
  #'  -output: return binary detection data
  ...
  summary(rowSums(eff_graze1820, na.rm = TRUE)); sd(rowSums(eff_graze1820, na.rm = TRUE))/nrow(eff_graze1820) 
  summary(rowSums(eff_hunt1820, na.rm = TRUE)); sd(rowSums(eff_hunt1820, na.rm = TRUE))/nrow(eff_hunt1820) 
